...
void quantize_row_tq2_0_128(const float * GGML_RESTRICT x, void * GGML_RESTRICT vy, int64_t k) {
    ...
    quantize_row_tq2_0_128_ref(x, y, k);
}
...
void ggml_vec_dot_tq2_0_128_q8_K_generic(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, size_t bx, const void * GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    // Prevent case when n < 256, then we only have half a block for input activation
    GGML_ASSERT(n % QK_K == 0);
    ...
    for (int i = 0; i < nb; ++i) {
        for (int b = 0; b < 2; ++b) {
            int32_t sumi = 0;
            const block_tq2_0_128 * xb = &x[2*i + b];

            for (size_t j = 0; j < sizeof(xb->qs); j += 32) {
                for (size_t l = 0; l < 4; ++l) {
                    for (size_t k = 0; k < 32; ++k) {
                        sumi += y[i].qs[b*128 + j*4 + l*32 + k] * (((xb->qs[j + k] >> (l*2)) & 3) - 1);
                    }
                }
            }
...
void ggml_vec_dot_tq2_0_128_q8_0_generic(
        int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, size_t bx,
        const void * GGML_RESTRICT vy, size_t by, int nrc) {
    assert(nrc == 1);
    UNUSED(nrc);
...
void ggml_vec_dot_tq2_0_128_q8_1_generic(
        int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, size_t bx,
        const void * GGML_RESTRICT vy, size_t by, int nrc) {
    assert(nrc == 1);
    UNUSED(nrc);
...
void ggml_vec_dot_tq2_0_128_q8_K(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, size_t bx, const void * GGML_RESTRICT vy, size_t by, int nrc) {
    ggml_vec_dot_tq2_0_128_q8_K_generic(n, s, bs, vx, bx, vy, by, nrc);
}
...
void ggml_vec_dot_tq2_0_128_q8_1(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, size_t bx, const void * GGML_RESTRICT vy, size_t by, int nrc) {
    ggml_vec_dot_tq2_0_128_q8_1_generic(n, s, bs, vx, bx, vy, by, nrc);
}
