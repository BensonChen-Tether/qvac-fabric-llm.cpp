...
void quantize_row_tq2_0_128_ref(const float * GGML_RESTRICT x, block_tq2_0_128 * GGML_RESTRICT y, int64_t k) {
    assert(k % QK_TQ2_0_128 == 0);
    const int64_t nb = k / QK_TQ2_0_128;

    for (int64_t i = 0; i < nb; i++) {
        float amax = 0.0f;

        for (int j = 0; j < QK_TQ2_0_128; j++) {
            const float v = x[j];
            amax = MAX(amax, fabsf(v));
        }
...
size_t quantize_tq2_0_128(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    (void)quant_weights; // not used
    const size_t row_size = ggml_row_size(GGML_TYPE_TQ2_0_128, n_per_row);
    quantize_row_tq2_0_128_ref(src, (block_tq2_0_128 *) dst, (int64_t)nrow*n_per_row);
    return nrow * row_size;
}
...
void dequantize_row_tq2_0_128(const block_tq2_0_128 * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    assert(k % QK_TQ2_0_128 == 0);
    const int64_t nb = k / QK_TQ2_0_128;
...
bool ggml_validate_row_data(enum ggml_type type, const void * data, size_t nbytes) {
    ...
    switch (type) {
        ...
        case GGML_TYPE_TQ2_0_128:
            {
                VALIDATE_ROW_DATA_D_F16_IMPL(block_tq2_0_128, data, nb);
            } break;
