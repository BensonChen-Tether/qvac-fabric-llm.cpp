...
// TQ2_0 packs four ternary values per byte.  The CUDA MMVQ path consumes
// Q8_1 activations, matching the upstream CUDA-TQ2_0 implementation.
...
static __device__ __forceinline__ float vec_dot_tq2_0_q8_1_impl(
        ...
        const float d2, const float * __restrict__ d8) {
    float sumf = 0.0f;
...
#pragma unroll
        for (int i = 0; i < vdr; ++i) {
            ...
            sumi = ggml_cuda_dp4a(__vsub4(vi, 0x01010101), u[vdr*i0 + i], sumi);
        }
        sumf += d8[i0] * sumi;
    }
