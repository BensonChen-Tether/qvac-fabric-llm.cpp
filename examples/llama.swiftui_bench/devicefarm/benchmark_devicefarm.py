#!/usr/bin/env python3
"""Collect AWS Device Farm iOS logs and generate an Excel benchmark report."""
...
try:
    ...
    print("Missing dependency: pip3 install -r devicefarm/requirements.txt", file=sys.stderr)
    raise
...
def infer_device_name(path: Path, meta: Optional[Dict[str, Any]]) -> str:
    ...
    for part in reversed(path.parts):
        if re.search(r"iPhone|Apple", part):
            return part
...
def collect_rows(results_dir: Path) -> List[Dict[str, Any]]:
    ...
    for path in results_dir.rglob("*"):
        if not path.is_file() or _is_under(path, extracted_dir):
            continue
        name = path.name.lower()
        if path.suffix.lower() in {".txt", ".log", ".logcat"} or "syslog" in name or "log" in name:
            candidate_files.append(path)

    meta_files = [p for p in results_dir.rglob("benchmark_meta.json") if not _is_under(p, extracted_dir)]
    result_files = [p for p in results_dir.rglob("benchmark_result.json") if not _is_under(p, extracted_dir)]
    ...
    for file_path in sorted(candidate_files, key=lambda p: (len(p.parts), str(p))):
        ...
        key = f"{file_path.parent.name}|{model_path}|{n_gpu}"
        if key in seen_keys:
            continue
        seen_keys.add(key)
    ...
    for meta_path in meta_files:
        ...
        if not isinstance(meta, dict) or bench is None:
            continue
        key = f"{meta_path.parent.name}|{meta.get('model_path')}|{meta.get('n_gpu_layers')}"
        if key in seen_keys:
            continue
        seen_keys.add(key)
        metrics = extract_metrics(bench)
...
def write_excel(rows: List[Dict[str, Any]], output_path: Path) -> None:
    ...
    if not df.empty:
        df = df.sort_values(by=["model_path", "n_gpu_layers", "device"], na_position="last")
    df.to_excel(output_path, index=False)


def main() -> int:
    parser = argparse.ArgumentParser(description="Generate Excel report from Device Farm iOS results")
