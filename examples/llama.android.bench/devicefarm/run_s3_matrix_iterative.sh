#!/usr/bin/env bash
# Run GPU+CPU Device Farm benchmarks for every model in s3-model-list.txt (Android).
...
for path in sorted(results_dir.rglob("*")):
    ...
    if "logcat" not in name and "syslog" not in name:
        continue
...
m = re.search(r"LLAMA_BENCH_RESULT\s+(\[.*?\])", text, re.DOTALL)
if not m:
    print("\t\t")
    raise SystemExit(0)
...
DEVICE_LABEL="${DEVICE_LABEL:-Google Pixel 10 Pro (Vulkan)}"
