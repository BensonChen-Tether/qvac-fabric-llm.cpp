...
Schedule an Android instrumentation benchmark on AWS Device Farm.
...
Environment:
  ...
  N_GPU_LAYERS        999=GPU Vulkan, 0=CPU (default: 999)
EOF
}
...
TIMEOUT_MIN="${TEST_TIMEOUT_MINUTES:-90}"
TEST_FILTER="${TEST_FILTER:-com.example.llama.bench.BenchmarkInstrumentedTest#runSelectedModelBenchmark}"
...
presign_model_url() {
  ...
  if ! presign_out="$(AWS_PROFILE="$AWS_S3_PROFILE" aws s3 presign "$s3_uri" --expires-in "$PRESIGN_EXPIRES" --region "$S3_REGION" 2>&1)"; then
    echo "Cannot access model: failed to presign $s3_uri" >&2
...
echo "Presigning S3 model URL for $MODEL_FILENAME..." >&2
...
echo "Building APKs with baked presigned URL (n_gpu_layers=$N_GPU_LAYERS)..." >&2
"$DEVICEFARM_DIR/build_devicefarm.sh"

APP_APK="${APP_APK:-$BUILD_DIR/app-debug.apk}"
TEST_APK="${TEST_APK:-$BUILD_DIR/app-debug-androidTest.apk}"
...
wait_for_upload() {
  ...
  if [[ -z "$arn" || "$arn" == "None" ]]; then
    echo "Upload did not return an ARN" >&2
    exit 1
  fi
  local status=""
...
echo "Uploading app APK..." >&2
APP_UPLOAD_ARN="$(upload_file "$APP_APK" "ANDROID_APP")"
echo "Uploading instrumentation test APK..." >&2
TEST_UPLOAD_ARN="$(upload_file "$TEST_APK" "INSTRUMENTATION_TEST_PACKAGE")"

export PROJECT_ARN DEVICE_ARN APP_UPLOAD_ARN TEST_UPLOAD_ARN TIMEOUT_MIN TEST_FILTER
...
if [[ "$WAIT_FOR_RUN" -eq 0 ]]; then
  echo ""
  echo "Run scheduled. Fetch when complete:"
...
while [[ "$STATUS" != "COMPLETED" ]]; do
  ...
  if ! poll="$(aws devicefarm get-run --region "$REGION" --arn "$RUN_ARN" --query 'run.[status,result]' --output text 2>&1)"; then
    ...
    echo "Fetch later: ./devicefarm/fetch_devicefarm_results.sh $RUN_ARN $RESULTS_DIR" >&2
    exit 1
  fi
